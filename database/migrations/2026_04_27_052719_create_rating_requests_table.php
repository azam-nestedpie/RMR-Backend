...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('rating_requests', function (Blueprint $table) {
            ...
            $table->string('requested_by_manager_firebase_uid', 128)->nullable();
            $table->string('rater_firebase_uid', 128);
            $table->string('subject_rep_firebase_uid', 128);
            $table->unsignedBigInteger('requested_by_role_id');
            ...
            $table->index('requested_by_manager_firebase_uid');
            $table->index('rater_firebase_uid');
            $table->index('subject_rep_firebase_uid');
            $table->index('status_id');
            $table->index(['requester_firebase_uid', 'target_user_firebase_uid', 'status_id'], 'rating_requests_pair_status_idx');
            $table->index(['requester_firebase_uid', 'target_user_firebase_uid', 'requested_at'], 'rating_requests_pair_requested_idx');
            ...
            $table->foreign('requested_by_manager_firebase_uid')->references('firebase_uid')->on('users')->nullOnDelete();
            $table->foreign('rater_firebase_uid')->references('firebase_uid')->on('users')->onDelete('cascade');
            $table->foreign('subject_rep_firebase_uid')->references('firebase_uid')->on('users')->onDelete('cascade');
